...
async def test_azure_openai_connection():
    """Test Azure OpenAI connection with proper method"""
    ...
    try:
        ...
        # Create chat history and test
        chat_history = ChatHistory()
        chat_history.add_user_message("Say 'Hello, Semantic Kernel is working!' if you can read this.")
        
        # Get response using the chat service directly
        ...
        if response and len(response) > 0:
            print(f"✓ Azure OpenAI responded: {response[0].content}")
            return True
        else:
...
async def main():
    ...
    if not await test_semantic_kernel():
        all_passed = False
    ...
    if all_passed:
        ...
        print("  python examples/basic/simple_conversation.py")
    else:
